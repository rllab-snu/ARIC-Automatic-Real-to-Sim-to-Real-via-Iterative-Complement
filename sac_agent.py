...
class SACAgent():
    ...
    def __init__(self, params):
        ...
        self.actions_num = params['action_space_shape'] # action_space.shape[0]

        self.action_range = [
            # float(self.env_info['action_space'].low.min()),
            # float(self.env_info['action_space'].high.max())
            params['action_space_low'],
            params['action_space_high']
        ]
        ...
        net_config = {
            ...
            'obs_dim': params['observation_space_shape'],
            'action_dim': params['action_space_shape'],
        ...
        self.replay_buffer = experience.VectorizedReplayBuffer(
            ...
            params['observation_space_shape'],
            params['action_space_shape'],
        ...
        self.target_entropy = self.target_entropy_coef * -params['action_space_shape'][0]
    ...
    def base_init(self, base_name, config):
        ...
        self.rewards_shaper = config['reward_shaper']
        self.num_agents = 1 # self.env_info.get('agents', 1)
        self.obs_shape = self.params["observation_space_shape"] # self.observation_space.shape
    ...
    def play_steps(self, random_exploration = False):
        ...
        for s in range(self.num_steps_per_episode):
            ...
            if random_exploration:
                action = torch.rand((self.num_actors, params['action_space_shape']), device=self._device) * 2.0 - 1.0
            else:
                with torch.no_grad():
                    action = self.act(obs.float(), params['action_space_shape'], sample=True)
            ...
            # yes_timeouts = torch.logical_and(self.current_lengths == self.max_env_steps, self.current_rewards < 100.0)
            # no_timeouts = ~yes_timeouts
